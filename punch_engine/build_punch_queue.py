...
def read_mapping(mapping_csv_path: str):
    """
    mapping.csv expected columns (current Phase 2):
      badge,associateOID,workAssignmentItemID,payrollFileNumber,positionID

    Backward-compatible:
      - if workAssignmentItemID isn't present, will accept workAssignmentID
    """
    mapping = {}
    with open(mapping_csv_path, newline="", encoding="utf-8-sig") as f:
        ...
        for row in r:
            ...
            # Keep your existing behavior: require badge+aoid+waid
            if badge and aoid and waid:
                mapping[badge] = {
                    "associateOID": aoid,
                    "workAssignmentID": waid,   # internal key name kept for minimal diffs
                    "positionID": posid,        # ✅ added
                }
    return mapping
...
def main():
    ...
    for row in rows:
        ...
        # ✅ inject positionID into the queued event
